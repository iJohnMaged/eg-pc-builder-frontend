...
import { CreateBuildProgress } from "../../data/types";
...
const GreenSpinner = ({ loading }: { loading: boolean }) => {
  const classes = classNames({
    hidden: !loading,
    "flex-shrink-0 inline-block w-4 h-4 text-white fill-green-600 animate-spin":
      true,
  });
  return (
    <svg
...
const SaveButton = (props: Props) => {
  ...
  const classes = classNames({
    ...
    "bg-red-500 hover:bg-red-600 animate-shake focus:ring-red-300 cursor-not-allowed":
      createBuildProgress === CreateBuildProgress.Error,
  });
  return (
    <button className={classes} onClick={cb}>
      <GreenSpinner
        loading={createBuildProgress === CreateBuildProgress.Saving}
      />
      <div>{createBuildProgress}</div>
    </button>
  );
};
